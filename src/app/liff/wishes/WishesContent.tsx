...
type Wish = { id: string; title: string; description: string | null; is_anonymous: boolean; created_by_user: { display_name: string; picture_url: string | null } | null; interests: { id: string; user_id: string; users: { display_name: string; picture_url: string | null } }[] };
...
export default function WishesContent() {
  ...
  useEffect(() => {
    const fetchGroupId = async () => {
      try {
        ...
        if (data && data.length > 0) setGroupId(data[0].group_id); else { setFetchError('所属グループがありません'); setIsLoading(false); }
  ...
  useEffect(() => {
    const fetchWishes = async () => {
      if (!groupId) return;
      try { const res = await fetch(`/api/groups/${groupId}/wishes`); const data = await res.json(); if (Array.isArray(data)) setWishes(data); } catch (err) { console.error(err); }
  ...
  const toggleInterest = async (wishId: string, hasInterest: boolean) => {
    if (!profile) return;
    try {
      if (hasInterest) await fetch(`/api/wishes/${wishId}/interest?lineUserId=${profile.userId}`, { method: 'DELETE' });
      else await fetch(`/api/wishes/${wishId}/interest`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lineUserId: profile.userId }) });
      const res = await fetch(`/api/groups/${groupId}/wishes`); const data = await res.json(); if (Array.isArray(data)) setWishes(data);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-20">
      <header className="bg-white border-b border-slate-200 px-4 py-4"><div className="flex items-center gap-4"><Link href={`/liff?groupId=${groupId}`} className="text-slate-400"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></Link><h1 className="text-lg font-semibold text-slate-900">行きたいリスト</h1></div></header>
      <main className="px-4 py-6">
        {wishes.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-6 text-center"><p className="text-slate-400 mb-4">まだ候補がありません</p><Link href={`/liff/wishes/new?groupId=${groupId}`} className="inline-block px-6 py-2.5 bg-slate-900 text-white text-sm font-medium rounded-lg">最初の候補を追加</Link></div>
        ) : (
          <div className="space-y-3">{wishes.map((wish) => { const hasInterest = wish.interests.some((i) => i.users?.display_name === profile?.displayName); return (
