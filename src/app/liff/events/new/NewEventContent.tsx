...
export default function NewEventContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/groups/${groupId}/events`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: title.trim(), description: description.trim() || null, candidateDates: selectedDates.sort(), lineUserId: profile.userId }) });
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <form onSubmit={handleSubmit} className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4"><label className="block text-sm font-medium text-slate-700 mb-2">詳細（任意）</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="例: 19時以降でお願いします" rows={2} className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm resize-none" /></div>
        ...
        {selectedDates.length > 0 && <div className="bg-white rounded-xl border border-slate-200 p-4"><p className="text-sm font-medium text-slate-700 mb-2">選択中の日程</p><div className="flex flex-wrap gap-2">{selectedDates.sort().map((dateStr) => { const date = new Date(dateStr); return <span key={dateStr} className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 text-sm rounded-lg">{date.getMonth() + 1}/{date.getDate()}({weekdays[date.getDay()]})<button type="button" onClick={() => toggleDate(dateStr)}>×</button></span>; })}</div></div>}
